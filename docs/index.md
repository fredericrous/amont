...
`amont install` in a repository, and your next commit is checked by thirty-two
...
- **Useful in the first minute.** Other hook managers install empty and wait
  for you to write YAML. amont ships thirty-two checks — commit-message
...
1. **[Installing and activating](install.md)** — get the binary, turn hooks on
   ...
   again.
2. **[The checks](checks.md)** — what the thirty-two built-ins do, and what each
